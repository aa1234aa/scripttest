package com.bitnei.cloud.sys.util.translate;

import org.apache.commons.lang.StringUtils;

import java.util.HashMap;
import java.util.Map;

import static com.bitnei.cloud.sys.util.translate.DataFormatter.TRANSLATE_POSTFIX;

/**
 * <p>
 * ----------------------------------------------------------------------------- <br>
 * 工程名 ： data-access-service <br>
 * 功能： 请完善功能说明 <br>
 * 描述： 这个人很懒，什么都没有留下 <br>
 * 授权 : (C) Copyright (c) 2018 <br>
 * 公司 : 北京理工新源信息技术有限公司<br>
 * ----------------------------------------------------------------------------- <br>
 * 修改历史 <br>
 * <table width="432" border="1">
 * <tr>
 * <td>版本</td>
 * <td>时间</td>
 * <td>作者</td>
 * <td>改变</td>
 * </tr>
 * <tr>
 * <td>1.0</td>
 * <td>2019-03-09</td>
 * <td>chenpeng</td>
 * <td>创建</td>
 * </tr>
 * </table>
 * <br>
 * <font color="#FF0000">注意: 本内容仅限于[北京理工新源信息技术有限公司]内部使用，禁止转发</font> <br>
 *
 * @author chenpeng
 * @version 1.0
 * @since JDK1.8
 */
@Translate(dataSeqNos = {"2308"}, ruleEnums = {RuleTypeEnum.GB, RuleTypeEnum.DB})
public class D2308Translate implements ITranslate {
    /**
     * 解析
     *
     * @param dataSeqNo    数据项序号
     * @param value        值
     * @param ruleTypeEnum 协议编号
     * @return
     */
    @Override
    public Map<String, String> translate(String dataSeqNo, String value, RuleTypeEnum ruleTypeEnum) {


        Map<String, String> resultMap = new HashMap<>();

        String[] arr = value.trim().split("\\|");
        for (int k = 0; k < arr.length; k++) {
            String[] items = arr[k].split(",");
            for (int i = 0; i < items.length; i++) {
                String item = items[i];
                String[] das = item.split(":");
                if (das.length == 2) {
                    String key = das[0];
                    String v = das[1];
                    if ("2310".equals(key)) {
                        String dk = key + TRANSLATE_POSTFIX;
                        switch (v) {
                            case "1":
                                items[i] += ("," + dk + ":耗电");
                                break;
                            case "2":
                                items[i] += ("," + dk + ":发电");
                                break;
                            case "3":
                                items[i] += ("," + dk + ":关闭状态");
                                break;
                            case "4":
                                items[i] += ("," + dk + ":准备状态");
                                break;
                            case "254":
                                items[i] += ("," + dk + ":异常");
                                break;
                            default:
                                items[i] += ("," + dk + ":无效");
                        }
                    }
                }

            }
            arr[k] = org.apache.commons.lang3.StringUtils.join(items, ",");

        }
        resultMap.put(dataSeqNo, StringUtils.join(arr, "|"));
        return resultMap;
    }
}
