...
import com.bitnei.cloud.sys.dao.VehNotCanMapper;
...
import com.bitnei.cloud.sys.service.IVehNotCanService;
...
* ----------------------------------------------------------------------------- <br>
* 工程名 ： 基础框架 <br>
* 功能： VehNotCanService实现<br>
* 描述： VehNotCanService实现<br>
...
@Slf4j
@Service
@Mybatis(namespace = "com.bitnei.cloud.sys.dao.VehNotCanMapper" )
public class VehNotCanService extends BaseService implements IVehNotCanService, IOfflineExportCallback {
    ...
    @Autowired
    private VehNotCanMapper vehNotCanMapper;
   ...
   @Override
    public Object list(PagerInfo pagerInfo) {
       ...
       if (params.get("days") != null && !String.valueOf(params.get("days")).equals("")){
           ...
           if(!days.matches("^[0-9]+([.][0-9])?$")){
               throw new BusinessException("无CAN天数请输入正确的数值，并且小数点后只能保留一位");
           }
        ...
        if (pagerInfo.getLimit() == null ||  pagerInfo.getLimit()<0){
            ...
            List<VehNotCan> entries = findBySqlId("pagerModel", params);
            List<VehNotCanModel> models = new ArrayList();
            for(VehNotCan entry: entries){
                VehNotCan obj = (VehNotCan)entry;
                VehNotCanModel data = VehNotCanModel.fromEntry(obj);
        ...
        //分页查询
            ...
            // 进行分页查询
            final Page<VehNotCan> page = vehNotCanMapper.pagerModel(params, pageRowBounds);
            ...
            pr.setData(page.stream().map(
                    p -> {
                        ...
                        VehNotCanModel model = VehNotCanModel.fromEntry(p);
    ...
    public Object realTimeList(PagerInfo pagerInfo) {
        ...
        if (pagerInfo.getLimit() == null ||  pagerInfo.getLimit()<0){
            ...
            List<VehNotCan> entries = findBySqlId("findByRealTime", params);
            List<VehNotCanModel> models = new ArrayList();
            for(VehNotCan entry: entries){
                VehNotCan obj = (VehNotCan)entry;
                VehNotCanModel data = VehNotCanModel.fromEntry(obj);
    ...
    @Override
    public VehNotCanModel get(String id) {

        //获取当权限的map
        Map<String,Object> params = DataAccessKit.getAuthMap("sys_veh_not_can", "vnc");
        params.put("id",id);
        VehNotCan entry = unique("findById", params);
        if (entry == null){
            throw new BusinessException("对象已不存在");
        }
        return VehNotCanModel.fromEntry(entry);
    }

    @Override
    public VehNotCanModel getByVid(String vid) {

        //获取当权限的map
        Map<String,Object> params = DataAccessKit.getAuthMap("sys_veh_not_can", "vnc");
        params.put("uuid",vid);
        VehNotCan entry = unique("getByVid", params);
        if (entry == null){
            throw new BusinessException("对象已不存在");
        }
        return VehNotCanModel.fromEntry(entry);
    ...
    @Override
    public void insert(VehNotCanModel model) {

        VehNotCan obj = new VehNotCan();
    ...
    @Override
    public void update(VehNotCanModel model) {
    ...
    */
    ...
    public void export(PagerInfo pagerInfo) {
       ...
       //获取权限sql
        ...
        if (pagerInfo.getConditions() == null || pagerInfo.getConditions().size() == 0){
            ...
            new ExcelExportHandler<VehNotCan>(this, "pagerModel", params, "sys/res/vehNotCan/export.xls", "历史无CAN车辆记录表") {
                @Override
                public Object process(VehNotCan entity) {
    ...
    public String exportOffline(@NotNull final PagerInfo pagerInfo) {
        ...
        // 构建给回调方法的第 3 个参数
        final String exportFilePrefixName = "历史无CAN车辆记录";
    ...
    public void exportOfflineProcessor(
        ...
        if (params.get("days") != null && !String.valueOf(params.get("days")).equals("")) {
            String days = String.valueOf(params.get("days"));
            if (!days.matches("^[0-9]+([.][0-9])?$")) {
                throw new ExportErrorException("无CAN天数请输入正确的数值，并且小数点后只能保留一位");
            }
            if (StringUtil.isNotEmpty(days)) {
                days = Float.valueOf(days) * (24 * 60 * 60) + "";
            }
        ...
        final String excelTemplateFile = "sys/res/vehNotCan/export.xls";
        ...
        MybatisOfflineExportHandler.csv(
            ...
            exportFileName,
            excelTemplateFile,
            this.vehNotCanMapper::pagerModel,
            params,
    ...
    @NotNull
    private List<VehNotCanModel> fromEntityToModel(final @NotNull List<VehNotCan> entities) {

        final ArrayList<VehNotCanModel> models = Lists.newArrayList();
        for (final VehNotCan entity : entities) {

            final VehNotCanModel model = new VehNotCanModel();
            ...
            model.setShowLastUploadTime(model.getRegainUploadTime() == null ? null : DateUtil.getDate(model.getLastUploadTime().getTime()));
            model.setShowRegainUploadTime(model.getRegainUploadTime() == null ? null : DateUtil.getDate(model.getRegainUploadTime().getTime()));
    ...
    public void realTimeExport(PagerInfo pagerInfo) {
        ...
        new ExcelExportHandler<VehNotCan>(this, "findByRealTime", params, "sys/res/vehNotCan/realTimeExport.xls", "实时无CAN车辆记录表") {
            @Override
            public Object process(VehNotCan entity) {
                entity.setDays(entity.getSeconds() == null ? null : secondsToDays(entity.getSeconds()));
                entity.setShowLastUploadTime(entity.getLastUploadTime() == null ? null : DateUtil.getDate(entity.getLastUploadTime().getTime()));
    ...
    public void batchImport(MultipartFile file) {
        ...
        String messageType = "VEHNOTCAN"+ WsMessageConst.BATCH_IMPORT_SUFFIX;

        new ExcelBatchHandler<VehNotCanModel>(file, messageType, GroupExcelImport.class) {
             ...
             */
            @Override
            public List<String> extendValidate(VehNotCanModel model) {
                return null;
            }
             ...
             */
            @Override
            public void saveObject(VehNotCanModel model) {
            	insert(model);
            }
        }.work();
    ...
    public void batchUpdate(MultipartFile file) {
        ...
        String messageType = "VEHNOTCAN"+ WsMessageConst.BATCH_UPDATE_SUFFIX;

        new ExcelBatchHandler<VehNotCanModel>(file, messageType, GroupExcelUpdate.class) {
