...
*/
@Slf4j
@Api(value = "无CAN车辆记录表", description = "无CAN车辆记录表",  tags = {"监控中心-异常车辆-无CAN车辆记录表"})
@Controller
@RequestMapping(value = "/"+ Version.VERSION_V1+"/sys")
public class VehNotCanController{

    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    /** 模块基础请求前缀 **/
    public static final String BASE_AUTH_CODE ="VEHNOTCAN";
    /** 查看 **/
    ...
    @Autowired
    private IVehNotCanService vehNotCanService;
    ...
    @GetMapping(value = "/vehNotCans/{id}")
    @ResponseBody
    @RequiresPermissions(AUTH_DETAIL)
    public ResultMsg get(@PathVariable String id){

        VehNotCanModel vehNotCan = vehNotCanService.get(id);
        return ResultMsg.getResult(vehNotCan);
    }
    ...
    @GetMapping(value = "/vehNotCans/vid/{vid}")
    @ResponseBody
    @RequiresPermissions(AUTH_DETAIL)
    public ResultMsg getByVid(@PathVariable String vid){

        VehNotCanModel vehNotCan = vehNotCanService.getByVid(vid);
        return ResultMsg.getResult(vehNotCan);
    }
    ...
    public ResultMsg list(@RequestBody @Validated PagerInfo pagerInfo, BindingResult bindingResult){
        ...
        Object result = vehNotCanService.list(pagerInfo);
    ...
    @PostMapping(value = "/vehNotCans/realTime")
    @ResponseBody
    @RequiresPermissions(AUTH_LIST)
    public ResultMsg realTimeList(@RequestBody @Validated PagerInfo pagerInfo, BindingResult bindingResult){
        ...
        Object result = vehNotCanService.realTimeList(pagerInfo);
        return ResultMsg.getResult(result);
    }
    ...
    */
    ...
    @PostMapping(value = "/vehNotCan")
    @ResponseBody
    @RequiresPermissions(AUTH_ADD)
    public ResultMsg add(@RequestBody @Validated({GroupInsert.class}) VehNotCanModel demo1, BindingResult bindingResult){

        if (bindingResult.hasErrors()){
            return ResultMsg.getSchemaMsg(bindingResult);
        }
        vehNotCanService.insert(demo1);
    ...
    */
    ...
    @PutMapping(value = "/vehNotCans/{id}")
    @ResponseBody
    @RequiresPermissions(AUTH_UPDATE)
    public ResultMsg update(@RequestBody @Validated({GroupUpdate.class}) VehNotCanModel demo1, BindingResult bindingResult){

        if (bindingResult.hasErrors()){
            return ResultMsg.getSchemaMsg(bindingResult);
        }
        vehNotCanService.update(demo1);
    ...
    @DeleteMapping(value = "/vehNotCans/{id}")
    @ResponseBody
    @RequiresPermissions(AUTH_DELETE)
    @SLog(action = "删除")
    public ResultMsg delete(@PathVariable String id){

        int count = vehNotCanService.deleteMulti(id);
    ...
    @GetMapping(value = "/vehNotCans/export/{params}")
    ...
    @PostMapping(value = "/vehNotCans/exportOffline")
    ...
    public ResultMsg exportOffline(@RequestBody @Validated PagerInfo pagerInfo, BindingResult bindingResult) throws IOException {
        ...
        final String taskId = vehNotCanService.exportOffline(pagerInfo);
    ...
    @GetMapping(value = "/vehNotCans/realTimeExport/{params}")
    @RequiresPermissions(AUTH_EXPORT)
    @ApiImplicitParam(name = "params", value = "查询参数json字符串，用urlEncoder编码", required = true, dataType = "String",paramType = "path")
    public void realTimeExport(@PathVariable String params) throws IOException {
        ...
        vehNotCanService.realTimeExport(info);
        return ;
