...
public class SimManagementResult extends BaseControllerTest {

    @Autowired
    private MockMvc mvc;

    public ResultActions getAddResult(String url, SimManagementModel simManagementModel) throws Exception {

        ResultActions   actions = mvc.perform(
                MockMvcRequestBuilders.post(url).headers(httpHeader())
                        .contentType(MediaType.APPLICATION_JSON).content(new ObjectMapper().writeValueAsString(simManagementModel)))
    ...
    public ResultActions getRegisterResult(String url, String id) throws Exception {

        ResultActions   actions = mvc.perform(
                MockMvcRequestBuilders.put(url,id).headers(httpHeader())
                        .contentType(MediaType.APPLICATION_JSON).content(new ObjectMapper().writeValueAsString(id)))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.code").value(200))
